...
class CommandLineParser(argparse.ArgumentParser):
    ...
    def add_projector_rebalance_arguments(self) -> None:
        ...
        projector_group.add_argument("--projector-rebalance-weights", type=str, default=None, help="Comma-separated per-layer diffs added to the model's learned txtfusion.projector weight (a Linear[layers->1] that fuses the tapped text-encoder layers into one conditioning) -- a reversible, LoRA-style model patch, not a per-generation conditioning scale. Length must match the model's number of tapped layers. Pass 'none' to disable the patch. Default: no patch; passing --projector-rebalance-strength alone activates the model's preset diffs (Krea 2: the identity-edit recipe values).")
        projector_group.add_argument("--projector-rebalance-strength", type=float, default=None, help="Scales --projector-rebalance-weights before adding it to the projector weight, and activates the model's preset diffs when --projector-rebalance-weights is not set. The diffs are typically large, so keep this small (default 0.05 when the patch is active); raise until output destabilizes.")
    ...
    def add_img_ref_arguments(self) -> None:
        ...
        img_ref_group.add_argument("--edit-ref", type=str, nargs="+", default=None, metavar="PATH", help="Up to 3 reference images for in-context edit LoRAs (ai-toolkit style): each image is VAE-encoded and appended to the DiT sequence as clean tokens (RoPE frame index distinguishes them from the target), and also fed to the text encoder's vision tower under the base template. The starting latent stays pure noise. Requires an edit-trained LoRA (--lora) to have any effect; mutually exclusive with --img-ref.")
        img_ref_group.add_argument("--edit-ref-boost", type=float, default=1.0, metavar="FLOAT", help="Reference-fidelity dial for --edit-ref: additive attention-logit bias that multiplies target->reference attention (all references equally). 1.0 = off/no effect (default, matches the reference workflow's 'classic v1.1' behavior), 4.0 = the reference workflow's recommended preset for the Identity Edit LoRA (much stronger face/body likeness), >10 tends to break removals/replacements, <1 loosens. Ignored when --edit-ref is not set.")
